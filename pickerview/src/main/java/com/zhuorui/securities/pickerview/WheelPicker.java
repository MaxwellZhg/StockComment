...
import com.zhuorui.securities.pickerview.util.LinearGradient;
...
public class WheelPicker<T> extends View {
	...
	private void initAttrs(Context context, @Nullable AttributeSet attrs) {
		...
		mTextColor = a.getColor(R.styleable.WheelPicker_itemTextColor, Color.parseColor("#A1A1A1"));
		...
		mSelectedItemTextColor = a.getColor(R.styleable.WheelPicker_selectedTextColor, Color.parseColor("#232323"));
        ...
        mCurtainColor = a.getColor(R.styleable.WheelPicker_wheelCurtainColor, Color.parseColor("#FFFFFF"));
	...
	public void computeTextSize() {
        ...
        if (!TextUtils.isEmpty(mItemMaximumWidthText)) {
        	...
        	T t = mDataList.get(0);
        	if (t instanceof IWheelData){
				mTextMaxWidth = (int) mPaint.measureText(((IWheelData)t).getItemText());
			}else {
				mTextMaxWidth = (int) mPaint.measureText(t.toString());
			}
		}
...
//			canvas.drawRect(mDrawnRect, mPaint);
		...
		for (int drawDataPos = drawnSelectedPos - mHalfVisibleItemCount - 1;
			...
			if (mDataFormat == null){
				drawText = data instanceof IWheelData ? ((IWheelData)data).getItemText() : data.toString();
			}else {
				drawText = mDataFormat.format(data);
			}
            //在中间位置的Item作为被选中的。
